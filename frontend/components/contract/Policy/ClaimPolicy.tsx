  ...
  const handleClaimPolicy = async () => {
    ...
    try {
      writeContract({
        ...
        abi: InsuranceManagerABI,
        functionName: 'claimPolicy',
        args: ['0x123'], // Corrected to a single element array as required by the type
        __mode: 'prepared', // Adding the __mode property as required by the type
        authorizationList: [], // Adding the authorizationList property as required by the type
      })
      toast.success('Policy claim request sent.')
    } catch (error) {
      console.error('Error claiming policy:', error)
